import { Check, Circle, Send, Edit3 } from "lucide-react";
import { useStore } from "../state/store";

export function KeyPointsPanel() {
  const { keyPoints, toggleKeyPoint, sendForApproval, skills, featuredSkillId, souls, activeSoulId } = useStore();
  ...
  return (
    <div className="rounded-3xl bg-white border border-white/60 shadow-sm p-4 flex flex-col gap-3">
      <header className="flex items-center justify-between">
        ...
        <button className="size-7 rounded-full hover:bg-neutral-100 flex items-center justify-center">
      ...
      <ul className="flex flex-col gap-1">
        {keyPoints.map((p) => (
          <li key={p.id}>
            ...
            >
              <span className={p.done ? "text-emerald-600" : "text-neutral-300"}>
                {p.done ? <Check className="size-4" /> : <Circle className="size-4" />}
              </span>
              <span className={`flex-1 text-sm ${p.done ? "text-neutral-400 line-through" : "text-neutral-700"}`}>
                {p.text}
              </span>

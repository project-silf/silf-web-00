import { Sparkles, TrendingUp, FileCheck2, Clock, Play, Filter } from "lucide-react";
  ...
  return (
    <div className="h-full bg-white/80 backdrop-blur rounded-3xl border border-white/60 shadow-sm p-5 overflow-y-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
          ...
          return (
            <div
              ...
              className={cn(
                ...
                active ? "border-neutral-900 shadow-md" : "border-neutral-100 hover:border-neutral-300"
              )}
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={cn("px-2 py-0.5 rounded-full text-xs flex items-center gap-1 shrink-0", meta.tone)}>
                  <Icon className="size-3" /> {meta.label}
                </span>
              </div>

              <div className="mt-3 h-2 rounded-full bg-neutral-100 overflow-hidden">
                <div className="h-full bg-gradient-to-r from-emerald-300 to-lime-400 transition-all" style={{ width: `${s.progress}%` }} />
              </div>
              ...
              <div className="mt-3 flex gap-2">
                ...
                <button
                  onClick={() => { featureSkill(s.id); runSkill(); }}
        ...
        {list.length === 0 && (
          <p className="text-sm text-neutral-500">No skills match this filter.</p>
        )}
      </div>
    </div>
  );

import { Building2, Users, UserRound, ChevronRight } from "lucide-react";
  ...
  return (
    <div className="w-full h-full bg-white/80 backdrop-blur rounded-3xl p-4 shadow-sm border border-white/60 flex flex-col gap-5 overflow-hidden">
      <header className="flex items-center justify-between">
        ...
        <button className="size-8 rounded-full bg-neutral-100 hover:bg-neutral-200 flex items-center justify-center">
              ...
              <ul className="flex flex-col gap-2">
                {list.map((s) => (
                  <li key={s.id}>
                    ...
                    >
                      <div className={cn("size-9 rounded-xl bg-gradient-to-br flex items-center justify-center text-neutral-800 shrink-0", s.accent)}>
                        <span className="text-xs">{s.initials}</span>
                      </div>
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-sm">{s.name}</p>
                        <p className={cn("truncate text-xs", activeSoulId === s.id ? "text-neutral-300" : "text-neutral-500")}>

  ...
  return (
    <div className="rounded-2xl bg-white border border-neutral-100 p-3">
      <div className="flex items-baseline justify-between mb-2">
        ...
        <p className="text-sm text-neutral-900">{last}{unit ? <span className="text-xs text-neutral-500 ml-0.5">{unit}</span> : null}</p>
      </div>
      <svg viewBox="0 0 100 100" preserveAspectRatio="none" className="w-full h-12">
        <path d={area} fill={fill[tone]} />
        <path d={line} fill="none" stroke={stroke[tone]} strokeWidth="2" vectorEffect="non-scaling-stroke" />
      </svg>
    </div>
  );
}
